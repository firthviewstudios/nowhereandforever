 ...
 * rename the js file.
 * --------------------------Latest Version------------------------------------
 * Hi all, with the change in 1.3.0 to use the default splash screen, some 
 * sprite behavior changed such as the automatic scaling and centering of the
 * splash sprites. This is due to the new default code, but I have heard some
 * feedback asking for the old way which did not have scaling for sprites. I
 * decided to add both an option to center and an option to scale the sprite
 * for each splash as a result, so you should once again be able to have your
 * sprites not scaled or centered.
 *
 * Version 1.3.1
 * - Added option to scale sprite
 * - Added option to center sprite
 *
 ...
 * @type boolean
 * @default true
 * @desc Scale the image to fit the screen?
 *
 * @param Video
 * @type file
 * @dir movies/
...
CGMZ.Versions["Splash Screen"] = "1.3.1";
CGMZ.SplashScreen = {};
...
CGMZ_Splash.prototype.initialize = function(splash) {
	...
	this.centerImage = (splash["Center Image"] === 'true');
	this.scaleImage = (splash["Scale Image"] === 'true');
...
Scene_Splash.prototype.CGMZ_loadSplash = function() {
	...
	this._backSprite.scale.x = 1;
	this._backSprite.scale.y = 1;
	this._backSprite.anchor.x = 0;
	this._backSprite.anchor.y = 0;
	...
	if(splash.isImageSplash()) {
		...
		bitmap.addLoadListener(this.CGMZ_onSplashLoaded.bind(this, bitmap, splash.centerImage, splash.scaleImage));
	} else {
...
//-----------------------------------------------------------------------------
Scene_Splash.prototype.CGMZ_onSplashLoaded = function(bitmap, center, scale) {
	this._backSprite.bitmap = bitmap;
	if(center) this.centerSprite(this._backSprite);
	if(scale) this.scaleSprite(this._backSprite);
	this._phase = 'fadeIn';
