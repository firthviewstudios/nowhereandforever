...
W.prototype.updateTabLabel = function () {
  ...
  if (this._tabLabel) {
    this._tabLabel.contents.clear();
    const text = labelMap[this._currentTab] + " | Press ←/→ to switch tabs";
    this._tabLabel.drawText(text, 0, 0, this._tabLabel.innerWidth, "center");
  }
};

W.prototype.createTabButtons = function () {
  const leftButton = new Sprite_Button("arrow-left");
  leftButton.x = 20;
  leftButton.y = Graphics.boxHeight * 0.1 - 40;
  leftButton.setClickHandler(() => {
    this._currentTabIndex = (this._currentTabIndex - 1 + this._tabs.length) % this._tabs.length;
    this._currentTab = this._tabs[this._currentTabIndex];
    this._questListWindow.setTab(this._currentTab);
    this.updateTabLabel(); this.createTabButtons();
  ...
  const rightButton = new Sprite_Button("arrow-right");
  rightButton.x = Graphics.boxWidth - 64;
  rightButton.y = Graphics.boxHeight * 0.1 - 40;
  rightButton.setClickHandler(() => {
    this._currentTabIndex = (this._currentTabIndex + 1) % this._tabs.length;
    this._currentTab = this._tabs[this._currentTabIndex];
    this._questListWindow.setTab(this._currentTab);
    this.updateTabLabel(); this.createTabButtons();
