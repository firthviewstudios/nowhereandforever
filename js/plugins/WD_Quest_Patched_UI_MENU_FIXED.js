...
Window_QuestTab.prototype.refresh = function() {
    this.contents.clear();
    const text = this._tab === "ongoing" ? "Active Quests" : "Completed Quests";
    const w = this.innerWidth;
    this.drawText(text, 0, 0, w, "center");
};

Window_QuestTab.prototype.isInsideTabArea = function(x, y) {
    return this.innerRect.contains(x, y);
};


W.prototype.createTabWindow = function() {
    const rect = new Rectangle(0, 0, Graphics.boxWidth, 48);
    this._questTabWindow = new Window_QuestTab(rect);
    this.addWindow(this._questTabWindow);
    this._questTabWindow.setTab(this._questTab);
};


// Add Tab key to key mapper
Input.keyMapper[9] = "tab";

// Extend the Scene_Quest update method to handle tab switching
...
W.prototype.checkToggleTab = function() {
    if (Input.isTriggered("left") || Input.isTriggered("right") || Input.isTriggered("tab")) {
        this.toggleQuestTab();
