...
(() => {
    ...
    function createOverlay() {
        ...
        function vpadShouldShowMenu() {
            // Show MENU only on the map (like the default touch UI),
            // but keep it visible during message/dialog boxes to avoid flicker.
        ...
        function openMenu(e) {
            ...
            // 1) Prevent stacking menus: ignore presses while already in a menu scene
            if (typeof Scene_MenuBase !== "undefined" && scene && (scene instanceof Scene_MenuBase)) return;

            // 2) Only allow opening the menu when the engine would normally allow it
            //    (prevents opening during running events, including in the gaps between dialog boxes)
            if (typeof Scene_Map === "undefined" || !(scene instanceof Scene_Map)) return;
            if (typeof scene.isMenuEnabled === "function" && !scene.isMenuEnabled()) return;

            // Extra safety: if a message is currently busy, don't open menu
